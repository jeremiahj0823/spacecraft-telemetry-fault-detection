...
fault detection and finite state machine mode management logic.

## Simulation

The simulator writes one row of data to a csv per second. Simulated telemetry includes the
following: bus voltage, battery percentage, temperature, and heartbeat. Noise is added to simulate real hardware, and clamps are imposed to ensure realistic bounds.
...
Fault detection is separate from the telemetry simulation. Telemetry data is read and flags are produced based on the data. All flags are included for the aforementioned faults.

## State Machine

The Finite State Machine keeps track of safe spacecraft operating modes:

Nominal: All systems okay

Degraded: Faults detected, non-critical, lowered activity

Safe Mode: Critical faults detected, heavily lowered activity
